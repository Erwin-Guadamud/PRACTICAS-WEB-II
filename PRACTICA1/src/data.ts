import { Paciente, SignoVital, ControlRealizado, DetallesConsulta } from './interfaces'
...
export const controlesRealizados: ControlRealizado[] = [
...
export const detallesConsulta: DetallesConsulta[] = []

pacientes.map(paciente => {
  const signosVitalesPaciente: SignoVital[] = signosVitales.filter(signoVital =>
    controlesRealizados.some(
      control => control.idPaciente === paciente.id && control.idSignoVital === signoVital.id
    )
  )

  const controlesRealizadosPaciente: ControlRealizado[] = controlesRealizados.filter(
    control => control.idPaciente === paciente.id
  )

  detallesConsulta.push({
    ...paciente,
